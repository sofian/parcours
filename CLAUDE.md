...
- Bilingual fields are paired columns (`title_fr` / `title_en`) applied consistently across tables.
...
- **Categories:** publications, grants, artworks, students, teaching, service, outreach, presentations, press, review, catalog, education, positions, recognitions, exhibitions, curatorship, residencies, software, skills. Full column-by-column detail lives in SPECS.md's "Category schemas" — don't duplicate it here; a few things worth remembering when touching this area:
  ...
  - `exhibitions`/`curatorship` are identical in shape but separate tables (exhibited-in vs. curated), not one table with a `role` column.
...
- **Cross-cutting mechanisms** (apply across many schemas, not category-specific):
  - `require_one_of` (schema-level, alongside `columns`/`dedup`): at least one of a group of columns must be filled, not all — used for `title_en`/`title_fr` pairs (most titles are single-language proper nouns) and `exhibitions`'/`curatorship`'s `event`/`venue`. Distinct from `labels.csv`'s stricter both-required rule, which only applies to the small fixed set of UI-facing strings.
  - Place names use one `location` column (city+country as a unit, e.g. "Berlin, Germany" — not split, since the city name itself can change between languages, e.g. The Hague/La Haye), backed by a `labels.csv` glossary (`category: location`) with literal-text fallback when unmatched.
  - `precision` on date columns is a **minimum**, never a ceiling — a column can always hold a more precise date than its declared floor.

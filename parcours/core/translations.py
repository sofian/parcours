...
the content glossary (e.g. `category: city`) — see SPECS.md,
...
def translations_csv_path(data_dir: Path) -> Path:
    return data_dir / "entries" / "translations.csv"
...
def add_translation(data_dir: Path, category: str, id_: str, en: str, fr: str) -> TranslationEntry:
    table = _load_translations_or_empty(translations_csv_path(data_dir))
    ...
    was_already_dirty = is_file_dirty(data_dir, "entries/translations.csv")
    write_all_rows(translations_csv_path(data_dir), _FIELDNAMES, rows)
    git_commit(data_dir, "entries/translations.csv", f"Added translation {category}:{id_}", was_already_dirty)
    return new_entry
...
def edit_translation(data_dir: Path, category: str, id_: str, en: str, fr: str) -> TranslationEntry:
    ...
    was_already_dirty = is_file_dirty(data_dir, "entries/translations.csv")
    write_all_rows(translations_csv_path(data_dir), _FIELDNAMES, rows)
    git_commit(data_dir, "entries/translations.csv", f"Edited translation {category}:{id_}", was_already_dirty)
    return updated
...
def delete_translation(data_dir: Path, category: str, id_: str) -> None:
    ...
    was_already_dirty = is_file_dirty(data_dir, "entries/translations.csv")
    write_all_rows(translations_csv_path(data_dir), _FIELDNAMES, rows)
    git_commit(data_dir, "entries/translations.csv", f"Deleted translation {category}:{id_}", was_already_dirty)

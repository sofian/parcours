...
def test_translation_add_creates_labels_csv_and_commits(tmp_path, monkeypatch):
    repo = _setup_data_repo(tmp_path)
    monkeypatch.chdir(repo)
    monkeypatch.setattr("parcours.core.labels.git_commit", lambda *a, **k: None)
    ...
    content = (repo / "labels.csv").read_text(encoding="utf-8")
...
def test_translation_list_reports_none_when_labels_csv_missing(tmp_path, monkeypatch):
    repo = _setup_data_repo(tmp_path)
    monkeypatch.chdir(repo)
